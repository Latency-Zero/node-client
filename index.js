...
class LatZeroClient extends EventEmitter {
    constructor(dsn, pool, options = {}) {
        ...
        this.process = {
             ...
             *
             * options.responseTo  — if set, result is delivered to that client (non-blocking).
             * options.timeout     — ms, defaults to client timeout.
             */
            call: (processId, data = {}, options = {}) => {
                ...
                // Blocking: wait for ack then app_result
                return new Promise((resolve, reject) => {
                    ...
                    const makeResultHandler = () => ({
                        resolve: (msg) => {
                            if (msg.type === 'ack') {
                                // Routing confirmed — re-arm to wait for app_result
                                _self.pending.set(requestId, makeResultHandler());
    ...
    connect() {
        return new Promise((resolve, reject) => {
            ...
            socket.on('connect', () => {
                console.log(`[LatZero] Connected to ${this.host}:${this.port}`);
                this.connected = true;
            ...
            socket.on('error', (err) => {
                console.error('[LatZero] Socket error:', err);
                this.connected = false;
                this.emit('error', err);
                reject(err);
            });
        ...
        for (const line of lines) {
            if (line.trim()) {
                try {
                    ...
                    console.error('[LatZero] Failed to parse message:', line, err);
                }
            }
        }
    }
